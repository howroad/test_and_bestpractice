if(document.getElementById("meizzCalendarLayer")==null){
...
function writeIframe()
{
    ...
    "    <table id=tableDay height=120 width=140 border=0 cellspacing=1 cellpadding=0>";
         ...
         " type=button value='&nbsp; &nbsp; 关闭' style='font-size:11px;font-family:宋体;' onclick='parent.clearCalendar()'></td></tr></table>"+
    ...
    "    </td><td align=center><input name=today type=button value='今天' onfocus='this.blur()' style='width: 50;font-size:11px;font-family:宋体;' title='当前日期&#13;快捷键：T'"+
...
function calendar(target) //主调函数
{

    var e = target;   writeIframe();
    ...
    try{
    	if(WebCalendar.initDate!=null){
       	    WebCalendar.thisYear   = WebCalendar.initDate.getFullYear();
                WebCalendar.thisMonth  = WebCalendar.initDate.getMonth()+ 1;
                WebCalendar.thisDay    = WebCalendar.initDate.getDate();
                writeCalendar();
       }else{
...
function clearCalendar(){WebCalendar.objExport.value="";};
...
function returnDate() //根据日期格式等返回用户选定的日期
{
    if(WebCalendar.objExport)
    {
        ...
        if(WebCalendar.objExport.onblur!=null)WebCalendar.objExport.onblur();
        if(WebCalendar.objExport.onchange!=null)WebCalendar.objExport.onchange();
        hiddenCalendar();
    }
}
...
function showCalendar(target,ButnID) {
 WebCalendar.darkColor  = "#61B877";    //控件的暗色
 WebCalendar.dayBgColor = "#E0EFD1";    //日期数字背景色
 WebCalendar.btnBgColor = "#DAECC8";    //控件的按钮背景色
 WebCalendar.todayColor = "#175A97";    //今天在日历上的标示背景色
 if(target.nodeName=="#text"){
 	target=target.previousSibling;
 }
 calendar(typeof target == 'string'?document.getElementById(target):target);
 if(event!=null)
