...
$(document).ready(function(){
	...
	window.setTimeout(function () {	//解决有些隐藏代码对布局的影响
		$.each($(".GridUnitDiv"), function (n, value) {
...
function smartPage_checkLength(obj,length){
	...
	alert(obj.alt+"输入不能超过"+length+"字符(一个中文为两个字符)");
...
function smartForm_addRow(obj) {
	try {
		...
		define = define
				+ " <input id='"
				+ count
				+ "' type='button' value='删除' onclick='smartForm_delRow(this)' name='"
				+ count + "' class='button4 button'> " + "</DIV>";
		$(div).append(define);
